    ...
    def forward_method(self, data):
        ...
        for layer in self.net:
            ...
            for node in layer:  # We iterate on each node on each layer of the network
                ...
                print('data', tmp_data, 'weight :', node['weights'], 'bias :', node['bias'])
    ...
    def backward_method(self, target):
        ...
        for i in reversed(range(n_layers)):
            ...
            for j, node in enumerate(layer):
                ...
                print ('relu', self.ReLU_derivative(node['output']))
        return loss
...
"A FAIRE :\n",
    ...
    - coder sequential module
    - linear module
    - coder minibatch + batch norm
    
    - faire la relu 
    
    - penalized term
    - coder cross entropy, minibatch, batch norm,dropout, 
    "- Normalisation L2\n",
